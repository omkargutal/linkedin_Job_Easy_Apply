'''
Author:     Omkar Santosh Gutal
LinkedIn:   https://www.linkedin.com/in/omkar-gutal-a25935249/
'''
...
#### Selenium Helper Decorators ####

def retry_on_stale_element(max_retries: int = 3, wait_time: float = 1.0):
    '''
    Decorator to retry function execution if StaleElementReferenceException occurs.
    - `max_retries`: Maximum number of retry attempts
    - `wait_time`: Time to wait (in seconds) between retries
    '''
    def decorator(func):
        @wraps(func)
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        print_lg(f"Stale element detected in {func.__name__}, retrying ({attempt + 1}/{max_retries - 1})...")
                        sleep(wait_time)
                    else:
                        print_lg(f"Failed to execute {func.__name__} after {max_retries} retries due to stale element")
                        return False
                except Exception as e:
                    raise e
        return wrapper
